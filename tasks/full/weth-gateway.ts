...
import { ConfettiUpgradeableProxy, WETHGateway } from "../../types";
...
task(`full:deploy-weth-gateway`, `Deploys the WETHGateway contract`)
  ...
  .setAction(async ({ verify, pool }, DRE) => {
    ...
    const initEncodedData = wethGatewayImpl.interface.encodeFunctionData(
      "initialize",
      [addressesProvider.address, weth]
    );

    let wethGateWay: WETHGateway;
    let wethGatewayProxy: ConfettiUpgradeableProxy;

    const wethGatewayAddress = undefined; //await addressesProvider.getAddress(ADDRESS_ID_WETH_GATEWAY);

    if (
      wethGatewayAddress != undefined &&
      notFalsyOrZeroAddress(wethGatewayAddress)
    ) {
      ...
      wethGatewayProxy = await getConfettiUpgradeableProxy(wethGatewayAddress);

      // only proxy admin can do upgrading
      await waitForTx(
        await proxyAdmin
          .connect(proxyAdminOwnerSigner)
          .upgrade(wethGatewayProxy.address, wethGatewayImpl.address)
      );
    ...
    console.log(
      "WETHGateway: proxy %s, implementation %s",
      wethGateWay.address,
      wethGatewayImpl.address
    );
...
task(
  "full:wethgateway-authorize-caller-whitelist",
