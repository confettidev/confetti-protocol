...
import { ConfettiUpgradeableProxy, PunkGateway } from "../../types";
...
task(`full:deploy-punk-gateway`, `Deploys the PunkGateway contract`)
  ...
  .setAction(async ({ verify, pool }, DRE) => {
    ...
    let punkGatewayProxy: ConfettiUpgradeableProxy;
    ...
    if (
      ...
      punkGatewayProxy = await getConfettiUpgradeableProxy(punkGatewayAddress);
    ...
    console.log(
      "PunkGateway: proxy %s, implementation %s",
      punkGateWay.address,
      punkGateWayImpl.address
    );
