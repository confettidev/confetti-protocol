...
import { ReserveOracle, ConfettiUpgradeableProxy } from "../../types";
...
task("full:deploy-oracle-reserve", "Deploy reserve oracle for full enviroment")
  ...
  .setAction(async ({ verify, skipOracle, pool }, DRE) => {
    try {
      ...
      const { ReserveAssets, ReserveAggregators } =
        poolConfig as ICommonConfiguration;

      const addressesProvider = await getLendPoolAddressesProvider();
      const reserveOracleAddress = getParamPerNetwork(
        poolConfig.ReserveOracle,
        network
      );

      if (skipOracle) {
        if (
          reserveOracleAddress == undefined ||
          !notFalsyOrZeroAddress(reserveOracleAddress)
        ) {
          throw Error("Invalid Reserve Oracle address in pool config");
        }
        console.log(
          "Reuse existed reserve oracle proxy:",
          reserveOracleAddress
        );
        await waitForTx(
          await addressesProvider.setReserveOracle(reserveOracleAddress)
        );
        return;
      }
      ...
      const reserveAggregators = getParamPerNetwork(
        ReserveAggregators,
        network
      );
      ...
      const initEncodedData = reserveOracleImpl.interface.encodeFunctionData(
        "initialize",
        [weth]
      );

      let reserveOracle: ReserveOracle;
      let reserveOracleProxy: ConfettiUpgradeableProxy;
      ...
      if (
        ...
        console.log(
          "Upgrading exist reserve oracle proxy to new implementation..."
        );
        ...
        reserveOracleProxy = await getConfettiUpgradeableProxy(
          reserveOracleAddress
        );
        ...
        await waitForTx(
          await proxyAdmin
            .connect(ownerSigner)
            .upgrade(reserveOracleProxy.address, reserveOracleImpl.address)
        );
        ...
        reserveOracleProxy = await deployConfettiUpgradeableProxy(
          eContractid.ReserveOracle,
          proxyAdmin.address,
        ...
        const oracleOwnerSigner = await getEthersSignerByAddress(
          await reserveOracle.owner()
        );
        await waitForTx(
          await reserveOracle
            .connect(oracleOwnerSigner)
            .setAggregators(tokens, aggregators)
        );
      }

      // Register the proxy oracle on the addressesProvider
      await waitForTx(
        await addressesProvider.setReserveOracle(reserveOracle.address)
      );

      console.log(
        "Reserve Oracle: proxy %s, implementation %s",
