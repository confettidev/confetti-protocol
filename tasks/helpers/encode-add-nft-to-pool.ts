...
import { strategyNftParams } from "../../markets/confetti/nftsConfigs";
...
task("encode-add-nft-to-pool", "Init and config new nft asset to lend pool")
  ...
  .setAction(async ({ pool, asset, strategy }, DRE) => {
    ...
    const cnftRegistryProxyAddress = await addressesProvider.getCNFTRegistry();
    const cnftRegistry = await getCNFTRegistryProxy(cnftRegistryProxyAddress);
    const { cNFTProxy } = await cnftRegistry.getCNFTAddresses(asset);
    if (cNFTProxy == undefined || !notFalsyOrZeroAddress(cNFTProxy)) {
      throw new Error("The CNFT of asset is not created");
    }
    ...
    const batchInitNftEncodeData =
      lendPoolConfiguratorProxy.interface.encodeFunctionData("batchInitNft", [
        initInputParams,
      ]);
    ...
    let cfgInputParams: {
    ...
    }[] = [
      {
        ...
        buyItNowDuration: nftParam.buyItNowDuration,
        redeemFine: nftParam.redeemFine,
        redeemThreshold: nftParam.redeemThreshold,
        minBuyItNowFine: nftParam.minBuyItNowFine,
      },
    ];
    const batchCfgNftEncodeData =
      lendPoolConfiguratorProxy.interface.encodeFunctionData("batchConfigNft", [
        cfgInputParams,
      ]);
...
task("encode-authorize-lendpool-nft", "WETH gateway authorize to lend pool")
  ...
  .setAction(async ({ pool, assets }, DRE) => {
    ...
    const authorizeLendPoolNFTEncodeData =
      wethGateway.interface.encodeFunctionData("authorizeLendPoolNFT", [
        assetSplits,
      ]);
    console.log(
      "EncodeData: authorizeLendPoolNFT: ",
      authorizeLendPoolNFTEncodeData
    );
