...
task("pool-admin:set-pause", "Doing lend pool pause task")
  ...
  .setAction(async ({ pool, state }, DRE) => {
    ...
    const emAdmin = await DRE.ethers.getSigner(
      await addressesProvider.getEmergencyAdmin()
    );
    ...
    await waitForTx(
      await lendPoolConfiguratorProxy.connect(emAdmin).setPoolPause(wantPause)
    );
...
task("pool-admin:update-reserves-config", "Doing lend pool reserve config task")
  ...
  .setAction(async ({ pool }, DRE) => {
    ...
    const reservesAssets = getParamPerNetwork(
...
task("pool-admin:set-reserve-active", "Doing Reserve active task")
  ...
  .setAction(async ({ pool, asset, state }, DRE) => {
    ...
    const pmAdmin = await DRE.ethers.getSigner(
    ...
    const reserveConfig =
      await confettiDataProvider.getReserveConfigurationData(asset);
    ...
    if (wantState) {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).activateReserve(asset)
      );
    } else {
      await waitForTx(
...
task("pool-admin:set-reserve-frozen", "Doing Reserve frozen task")
  ...
  .setAction(async ({ pool, asset, state }, DRE) => {
    ...
    if (wantState) {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).freezeReserve(asset)
      );
    } else {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).unfreezeReserve(asset)
      );
    }
...
task("pool-admin:set-nft-active", "Doing NFT active task")
  ...
  .setAction(async ({ pool, asset, state }, DRE) => {
    ...
    const curNftConfig = await confettiDataProvider.getNftConfigurationData(
      asset
    );
    ...
    if (wantState) {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).activateNft(asset)
      );
    } else {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).deactivateNft(asset)
      );
    }

    const newNftConfig = await confettiDataProvider.getNftConfigurationData(
      asset
    );
...
task("pool-admin:set-nft-frozen", "Doing NFT frozen task")
  ...
  .setAction(async ({ pool, asset, state }, DRE) => {
    ...
    if (wantState) {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).freezeNft(asset)
      );
    } else {
      await waitForTx(
        await lendPoolConfiguratorProxy.connect(pmAdmin).unfreezeNft(asset)
      );
    }
