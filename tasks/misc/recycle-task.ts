...
task("dev:recycle-pool-eths", "Doing recycle all ETHs in pool task")
  ...
  .setAction(async ({ pool, id, to }, DRE) => {
    ...
    const bwethToken = await getCToken(wethReserveData.cTokenAddress);
    ...
    const approved = await bayc.isApprovedForAll(
      deployerAddress,
      wethGateway.address
    );
    if (!approved) {
    ...
    await waitForTx(
      await wethGateway.borrowETH(availableBalance, bayc.address, id, to, "0")
    );
  });

task(
  "dev:recycle-signer-tokens",
  "Doing recycle all ETHs and Tokens in signers"
)
  .addParam("to", "Address of target user")
  .setAction(async ({ to }, DRE) => {
    ...
    for (const signer of allSigners) {
      ...
      for (const erc20Symbol of Object.keys(allMockERC20s)) {
        ...
        if (tokenBalance.gt(0)) {
          ...
          await waitForTx(
            await erc20Token.connect(signer).transfer(to, tokenBalance)
          );
        }
      }
    }
    ...
    for (const signer of allSigners) {
      ...
      for (const nftSymbol of Object.keys(allMockNfts)) {
        ...
        const tokenBalance = (
          await nftToken.balanceOf(signerAddress)
        ).toNumber();
        for (let index = 0; index < tokenBalance; index++) {
          const tokenId = await nftToken.tokenOfOwnerByIndex(
            signerAddress,
            index
          );
          ...
          await waitForTx(
            await nftToken
              .connect(signer)
              .transferFrom(signerAddress, to, tokenId)
          );
        }
      }
    }
