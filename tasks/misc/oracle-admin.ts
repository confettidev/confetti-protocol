...
task("oracle-admin:set-oracle-proxy", "Doing oracle admin task")
  ...
  .setAction(async ({ pool, type, proxy }, DRE) => {
    ...
    const ownerSigner = await getEthersSignerByAddress(
      await addressesProvider.owner()
    );

    if (type == 1) {
      await waitForTx(
        await addressesProvider.connect(ownerSigner).setNFTOracle(proxy)
      );
    } else if (type == 2) {
      await waitForTx(
        await addressesProvider.connect(ownerSigner).setReserveOracle(proxy)
      );
    } else {
...
task("oracle-admin:set-nft-assets", "Set new nft asset to oracle")
  ...
  .setAction(async ({ pool, assets }, DRE) => {
    ...
    await waitForTx(
      await nftOracle.connect(ownerSigner).setAssets(assetsArray)
    );
...
task("oracle-admin:set-price-feed-admin", "Doing oracle admin task")
  ...
  .setAction(async ({ pool, feedAdmin }, DRE) => {
    ...
    console.log(
      "Current PriceFeedAdmin:",
      await nftOracleProxy.priceFeedAdmin()
    );
...
task("oracle-admin:feed-init-nft-price", "Doing oracle admin task")
  ...
  .setAction(async ({ pool }, DRE) => {
    ...
    for (const nftSymbol of Object.keys(nftsAssets)) {
      ...
      await waitForTx(
        await nftOracleProxy
          .connect(feedAdminSigner)
          .setAssetData(nftsAssets[nftSymbol], price)
      );
    }
  });
...
task("oracle-admin:add-usd-eth-asset", "Doing oracle admin task")
  ...
  .setAction(async ({ pool }, DRE) => {
    ...
    const oracle = await getReserveOracle(
    ...
    const aggregators = getParamPerNetwork(
      poolConfig.ReserveAggregators,
      network
    );
    ...
    await waitForTx(
      await oracle
        .connect(ownerSigner)
        .addAggregator(USD_ADDRESS, aggregators["USD"])
    );
