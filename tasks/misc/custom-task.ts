...
task("dev:cryptopunks-init", "Doing CryptoPunks init task").setAction(
  async ({}, DRE) => {
...
task("dev:generate-subgraph-events", "Doing custom task")
  ...
  .setAction(async ({ pool, nftTokenId }, DRE) => {
    ...
    await waitForTx(
      await borrowerBayc.setApprovalForAll(lendPool.address, true)
    );
    await waitForTx(await borrowerBayc.mint(nftTokenId));
    await waitForTx(
      await borrowerPool.borrow(
        dai.address,
...
task("dev:deposit-eth", "Doing custom task")
  ...
  .setAction(async ({ pool, amount }, DRE) => {
    ...
    const bWeth = await getCToken(wethResData.cTokenAddress);

    await waitForTx(
      await wethGateway.depositETH(await signer.getAddress(), "0", {
    ...
    console.log(
      "bWETH Balance:",
      (await bWeth.balanceOf(signerAddress)).toString()
    );
  });
...
task("dev:withdraw-eth", "Doing custom task")
  ...
  .setAction(async ({ pool, amount }, DRE) => {
    ...
    if (allowance.lt(amountDecimals)) {
      await waitForTx(
        await bWeth.approve(wethGateway.address, MAX_UINT_AMOUNT)
      );
    }
    ...
    await waitForTx(
      await wethGateway.withdrawETH(amountDecimals, signerAddress)
    );
  });
...
task("dev:borrow-eth-using-punk", "Doing custom task")
  ...
  .setAction(async ({ pool, id, amount }, DRE) => {
    ...
    const isApproveOk = await wpunk.isApprovedForAll(
      signerAddress,
      punkGateway.address
    );
    if (!isApproveOk) {
    ...
    let amountDecimals: BigNumberish;
    if (amount == "-1") {
      const confettiDataProvider = await getConfettiProtocolDataProvider(
        await addressesProvider.getConfettiDataProvider()
      );
      const wethResData = await confettiDataProvider.getReserveData(
        weth.address
      );
      console.log(
        "WETH Available Liquidity:",
        wethResData.availableLiquidity.toString()
      );
    ...
    if (notFalsyOrZeroAddress(punkAddress)) {
      if (punkAddress == signerAddress) {
        ...
        console.log(
          `Punk address ${punkAddress} is not owner ${signerAddress}`
        );
        borrowMore = true;
      }
    } else {
    ...
    if (!borrowMore) {
      await waitForTx(
        await punk.offerPunkForSaleToAddress(id, "0", punkGateway.address)
      );

      console.log("borrow ETH at first time");
      const txBorrow = await waitForTx(
        await punkGateway.borrowETH(
          amountDecimals,
          id,
          await signer.getAddress(),
          "0"
...
task("dev:borrow-usdc-using-punk", "Doing custom task")
  ...
  .setAction(async ({ pool }, DRE) => {
    ...
    await waitForTx(
      await punk.offerPunkForSaleToAddress("5002", "0", punkGateway.address)
    );
    await waitForTx(
      await punkGateway.borrow(
        daiAddress,
        "100000000000000000000",
        "5002",
        await signer.getAddress(),
...
task("dev:repay-eth-using-punk", "Doing repay task")
  ...
  .setAction(async ({ pool, id, amount }, DRE) => {
    ...
    const punk = await getCryptoPunksMarket();
    const wpunkAddress = await getWrappedPunkTokenAddress(
      poolConfig,
      punk.address
    );
    ...
    if (amount == "-1") {
      ...
      const loanData = await confettiDataProvider.getLoanDataByCollateral(
        wpunk.address,
        id
      );
      console.log("Loan Borrow Amount:", loanData.currentAmount.toString());
      amountDecimals = new BigNumber(loanData.currentAmount.toString())
        .multipliedBy(1.1)
        .toFixed(0);
    } else {
      amountDecimals = await convertToCurrencyDecimals(weth.address, amount);
    ...
    await waitForTx(
      await punkGateway.repayETH(id, amountDecimals, { value: amountDecimals })
    );
...
task("dev:repay-eth-using-erc721", "Doing repay task")
  ...
  .setAction(async ({ pool, token, id, amount }, DRE) => {
    ...
    await waitForTx(
      await wethGateway.repayETH(token, id, amountDecimals, {
        value: amountDecimals,
      })
    );
  });
...
task("dev:debt-approve-delegate", "Doing custom task")
  ...
  .setAction(async ({ pool, asset }, DRE) => {
    ...
    const reserveToken = await confettiDataProvider.getReserveTokenData(asset);
    const debtToken = await getDebtToken(reserveToken.debtTokenAddress);
    ...
    await waitForTx(
      await debtToken.approveDelegation(wethGateway.address, MAX_UINT_AMOUNT)
    );
