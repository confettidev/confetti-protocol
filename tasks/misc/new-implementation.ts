...
task("dev:deploy-new-implementation", "Deploy new implementation")
  ...
  .setAction(async ({ verify, pool, contract, upgrade }, DRE) => {
    ...
    const providerOwnerSigner = await getEthersSignerByAddress(
      await addressesProviderRaw.owner()
    );
    ...
    if (contract == "LendPoolConfigurator") {
      ...
      console.log(
        "LendPoolConfigurator implementation address:",
        lendPoolCfgImpl.address
      );
    ...
    if (contract == "LendPoolLoan") {
      ...
      console.log(
        "LendPoolLoan implementation address:",
    ...
    if (contract == "ReserveOracle") {
      ...
      console.log(
        "ReserveOracle implementation address:",
    ...
    if (contract == "WETHGateway") {
      ...
      console.log(
        "WETHGateway implementation address:",
    ...
    if (contract == "PunkGateway") {
      ...
      console.log(
        "PunkGateway implementation address:",
    ...
    if (contract == "ConfettiProtocolDataProvider") {
      const contractImpl = await deployConfettiProtocolDataProvider(
        addressesProvider.address,
        verify
      );
      console.log(
        "ConfettiProtocolDataProvider implementation address:",
        contractImpl.address
      );

      if (upgrade) {
        await waitForTx(
          await addressesProvider.setConfettiDataProvider(contractImpl.address)
        );
      }
    }
    ...
    if (contract == "UiPoolDataProvider") {
      ...
      console.log(
        "UiPoolDataProvider implementation address:",
        contractImpl.address
      );

      if (upgrade) {
        await waitForTx(
          await addressesProvider.setUIDataProvider(contractImpl.address)
        );
      }
    }
    ...
    if (contract == "WalletBalancerProvider") {
      ...
      console.log(
        "WalletBalancerProvider implementation address:",
        contractImpl.address
      );

      if (upgrade) {
        await waitForTx(
          await addressesProvider.setWalletBalanceProvider(contractImpl.address)
        );
      }
    }

    if (contract == "ConfettiCollector") {
      const contractImpl = await deployConfettiCollector([], verify);
      console.log(
        "ConfettiCollector implementation address:",
        contractImpl.address
      );
    }
  });
...
task("dev:deploy-new-interest-rate", "Deploy new interest rate implementation")
  ...
  .setAction(
    async (
      { verify, pool, optUtilRate, baseRate, rateSlope1, rateSlope2 },
      DRE
    ) => {
      await DRE.run("set-DRE");
      ...
      const addressesProvider =
        addressesProviderRaw.connect(providerOwnerSigner);
...
export const rateStrategyWETH: IInterestRateStrategyParams = {
  ...
  variableRateSlope2: new BigNumber(1).multipliedBy(oneRay).toFixed(),
    ...
    */
      ...
      const optUtilRateInRay = new BigNumber(optUtilRate)
        .multipliedBy(oneRay)
        .toFixed();
      const baseRateInRay = new BigNumber(baseRate)
        .multipliedBy(oneRay)
        .toFixed();
      const rateSlope1InRay = new BigNumber(rateSlope1)
        .multipliedBy(oneRay)
        .toFixed();
      const rateSlope2InRay = new BigNumber(rateSlope2)
        .multipliedBy(oneRay)
        .toFixed();
...
task("dev:upgrade-implementation", "Update implementation to address provider")
  ...
  .setAction(async ({ pool, contract, proxy, impl }, DRE) => {
    ...
    const confettiProxy = await getConfettiUpgradeableProxy(proxy);

    const proxyAdmin = await getConfettiProxyAdminById(
    ...
    await waitForTx(
      await proxyAdmin
        ...
        .upgrade(confettiProxy.address, impl)
    );

    await insertContractAddressInDb(eContractid[contract], proxy);
  });

task("dev:upgrade-all-debtTokens", "Update implementation to debt token")
  .addFlag("verify", "Verify contracts at Etherscan")
  .addParam(
    "pool",
    `Pool name to retrieve configuration, supported: ${Object.values(
  ...
  .setAction(async ({ verify, pool }, DRE) => {
    ...
    const protocolDataProvider = await getConfettiProtocolDataProvider(
      await addressesProviderRaw.getConfettiDataProvider()
    );
    ...
    await waitForTx(
      await lendPoolConfigurator
        .connect(poolAdminSigner)
        .updateDebtToken(inputs)
    );
  });
