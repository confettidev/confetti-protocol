...
task("print-config", "Print config of all reserves and nfts")
  ...
  .setAction(async ({ pool }, localBRE) => {
    ...
    console.log(
      "Lend Pool Loan Proxy:",
      await addressesProvider.getLendPoolLoan()
    );
    console.log(
      "Lend Pool Configurator Proxy:",
      await addressesProvider.getLendPoolConfigurator()
    );
    console.log(
      "Reserve Oracle Proxy:",
      await addressesProvider.getReserveOracle()
    );
    console.log("NFT Oracle Proxy:", await addressesProvider.getNFTOracle());
    console.log(
      "CNFT Registry Proxy:",
      await addressesProvider.getCNFTRegistry()
    );
    console.log(
      "Incentives Controller:",
      await addressesProvider.getIncentivesController()
    );
    console.log("Confetti Data Provider:", protocolDataProvider.address);
    console.log(
      "UI Data Provider:",
      await addressesProvider.getUIDataProvider()
    );
    console.log(
      "Wallet Balance Provider:",
      await addressesProvider.getWalletBalanceProvider()
    );
    ...
    console.log(
      `  - MaxNumberOfReserves: ${await lendPoolProxy.getMaxNumberOfReserves()}`
    );
    console.log(
      `  - MaxNumberOfNfts: ${await lendPoolProxy.getMaxNumberOfNfts()}`
    );
    ...
    for (const [symbol, address] of Object.entries(reserveAssets)) {
      ...
      const reserveData =
        await protocolDataProvider.getReserveConfigurationData(address);
      const reserveTokenData = await protocolDataProvider.getReserveTokenData(
        address
      );
    ...
    const nftTokensFields = ["cNFTSymbol", "cNFTAddress"];
    const nftsAsset = getParamPerNetwork(
      poolConfig.NftsAssets,
      network as eNetwork
    );
    for (const [symbol, address] of Object.entries(nftsAsset)) {
      ...
      const nftData = await protocolDataProvider.getNftConfigurationData(
        address
      );
