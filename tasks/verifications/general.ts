...
task("verify:general", "Verify general contracts at Etherscan")
  ...
  .setAction(async ({ all, pool }, localDRE) => {
    ...
    const confettiCollectorImpl = await getConfettiCollectorImpl();
    ...
    const lendPoolProxy = await getConfettiUpgradeableProxy(lendPoolAddress);
    const lendPoolConfiguratorProxy = await getConfettiUpgradeableProxy(
      lendPoolConfiguratorAddress
    );
    const lendPoolLoanProxy = await getConfettiUpgradeableProxy(
      lendPoolLoanAddress
    );
    ...
    const proxyAdminFund = await getConfettiProxyAdminById(
    ...
    const proxyAdminPool = await getConfettiProxyAdminById(
    ...
    if (all) {
      ...
      // Confetti Data Provider
      console.log("\n- Verifying Confetti Data Provider...\n");
      await verifyContract(
    ...
    // ConfettiCollector Proxy
    console.log("\n- Verifying Collector...\n");
    const confettiCollectorProxy = await getConfettiCollectorProxy();
    const collectorProxyAdmin = await getConfettiProxyAdminById(
      eContractid.ConfettiProxyAdminFund
    );
    await verifyContract(
      eContractid.ConfettiCollector,
      confettiCollectorProxy,
      [
    ...
    await verifyContract(eContractid.ConfettiUpgradeableProxy, lendPoolProxy, [
      lendPoolImpl.address,
      addressesProvider.address,
      lendPoolImpl.interface.encodeFunctionData("initialize", [
        addressesProvider.address,
      ]),
    ]);
    ...
    await verifyContract(
      ...
      lendPoolConfiguratorProxy,
      [
        lendPoolConfiguratorImpl.address,
        addressesProvider.address,
        lendPoolConfiguratorImpl.interface.encodeFunctionData("initialize", [
          addressesProvider.address,
        ]),
      ]
    );
    ...
    await verifyContract(
      ...
      [
        ...
        addressesProvider.address,
        lendPoolLoanImpl.interface.encodeFunctionData("initialize", [
          addressesProvider.address,
        ]),
      ]
    );
    ...
    await verifyContract(eContractid.ConfettiUpgradeableProxy, wethGateway, [
    ...
    // await verifyContract(eContractid.ConfettiUpgradeableProxy, punkGateway, [
