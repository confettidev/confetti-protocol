...
task("dev:mint-top-punks", "Mint top sale punks for dev enviroment")
  ...
  .setAction(async ({ target, ids }, localBRE) => {
    ...
    await waitForTx(
      await punks.setInitialOwners(topSalePunkOwners, topSalePunkIndexs)
    );
...
task("dev:mint-top-tokens", "Mint top sale tokens for dev enviroment")
  ...
  .setAction(async ({ symbol, ids, target }, localBRE) => {
    ...
    for (const tokenId of topSaleTokenIds) {
      ...
      if (minterLimit < 0 || minterLimit >= 10) {
        for (; minterIndex < allSingers.length; minterIndex++) {
          ...
          const tmpLimit = (
            await erc721Token.mintCounts(minterAddress)
          ).toNumber();
          if (tmpLimit < 10) {
        ...
        console.log(
          "Minter balance:",
          (await minterSigner.getBalance()).toString()
        );
      }
        ...
        if (tokenOwner == minterAddress) {
          await waitForTx(
            await erc721Token
              ...
              .transferFrom(minterAddress, target, tokenId)
          );
        } else {
    ...
    console.log(
      "Total Balance of Target: %d",
      await erc721Token.balanceOf(target)
    );
  });
