...
makeSuite("PunkGateway-Liquidate", (testEnv: TestEnv) => {
  ...
  before("Initializing configuration", async () => {
    ...
    punkInitPrice = await testEnv.nftOracle.getAssetPrice(
  ...
  it("Borrow USDC and liquidate it", async () => {
    ...
    const nftCfgData = await dataProvider.getNftConfigurationData(
      wrappedPunk.address
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(liquidator.signer)
        .buyItNow(punkIndex, liquidateAmount, liquidator.address)
    );
  ...
  it("Borrow USDC and redeem it", async () => {
    ...
    await increaseTime(
      nftCfgData.redeemDuration.mul(ONE_DAY).sub(3600).toNumber()
    );
