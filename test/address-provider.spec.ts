...
makeSuite("LendPoolAddressesProvider", (testEnv: TestEnv) => {
  it("Test the accessibility of the LendPoolAddressesProvider", async () => {
    ...
    for (const contractFunction of [
      ...
      addressesProvider.setCNFTRegistry,
      ...
      addressesProvider.setConfettiDataProvider,
  ...
  it("Tests adding a proxied address with `setAddressAsProxy()`", async () => {
    ...
    expect(proxiedAddressSetReceipt.events[2].event).to.be.equal(
      "ProxyCreated"
    );
    expect(proxiedAddressSetReceipt.events[3].event).to.be.equal("AddressSet");
    expect(proxiedAddressSetReceipt.events[3].args?.id).to.be.equal(
      proxiedAddressId
    );
    expect(proxiedAddressSetReceipt.events[3].args?.newAddress).to.be.equal(
      mockLendPool.address
    );
  ...
  it("Tests adding a non proxied address with `setAddress()`", async () => {
    ...
    expect(nonProxiedAddressSetReceipt.events[0].event).to.be.equal(
      "AddressSet"
    );
    expect(nonProxiedAddressSetReceipt.events[0].args?.id).to.be.equal(
      nonProxiedAddressId
    );
    expect(nonProxiedAddressSetReceipt.events[0].args?.newAddress).to.be.equal(
      mockNonProxiedAddress
    );
    expect(nonProxiedAddressSetReceipt.events[0].args?.hasProxy).to.be.equal(
      false
    );
  });
});
