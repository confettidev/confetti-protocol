...
makeSuite("LendPool: Batch borrow test cases", (testEnv: TestEnv) => {
  ...
  it("Batch Borrow WETH using many NFTs", async () => {
    ...
    expect(
      ...
      "current weth balance shoud increase"
    ).to.be.eq(userBalanceBeforeBorrow.add(borrowAmount1).add(borrowAmount2));
    ...
    await waitForTx(
      await pool
        ...
        .batchRepay(
          [bayc.address, bayc.address],
          [tokenId1, tokenId2],
          [borrowAmount1.div(2), borrowAmount1.div(2)]
        )
    );

    const loanDataAfterRepayPart = await dataProvider.getLoanDataByCollateral(
      bayc.address,
      tokenId1
    );
