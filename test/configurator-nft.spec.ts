...
import { strategyNftClassB } from "../markets/confetti/nftsConfigs";
...
makeSuite("Configurator-NFT", (testEnv: TestEnv) => {
  ...
  it("Check the onlyAdmin on configureNftAsCollateral ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .configureNftAsCollateral([bayc.address], "7500", "8000", "500"),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });

  it("Deactivates the BAYC NFT as buyItNow", async () => {
    const { configurator, dataProvider, bayc } = testEnv;
    await configurator.configureNftAsBuyItNow([bayc.address], 0, 0, 0);
    await configurator.setNftRedeemThreshold([bayc.address], 0);
    await configurator.setNftMinBuyItNowFine([bayc.address], 0);
    ...
    expect(buyItNowDuration).to.be.equal(0);
    expect(redeemFine).to.be.equal(0);
    expect(redeemThreshold).to.be.equal(0);
    expect(minBuyItNowFine).to.be.equal(0);
  });

  it("Activates the BAYC NFT as buyItNow", async () => {
    const { configurator, dataProvider, bayc } = testEnv;
    await configurator.configureNftAsBuyItNow([bayc.address], "1", "1", "100");
    await configurator.setNftRedeemThreshold([bayc.address], "5000");
    await configurator.setNftMinBuyItNowFine([bayc.address], 5000);
    ...
    expect(buyItNowDuration).to.be.equal(1);
    expect(redeemFine).to.be.equal(100);
    expect(redeemThreshold).to.be.equal(5000);
    expect(minBuyItNowFine).to.be.equal(5000);
  });

  it("Check the onlyAdmin on configureNftAsBuyItNow ", async () => {
    const { configurator, users, bayc } = testEnv;
    await expect(
      configurator
        .connect(users[2].signer)
        .configureNftAsBuyItNow([bayc.address], "1", "1", "100"),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
    await expect(
      configurator
        .connect(users[2].signer)
        .setNftRedeemThreshold([bayc.address], "5000"),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Batch Deactivates the BAYC NFT as BuyItNow", async () => {
    const { configurator, dataProvider, bayc } = testEnv;

    cfgInputParams[0].asset = bayc.address;
    cfgInputParams[0].buyItNowDuration = 0;
    cfgInputParams[0].redeemDuration = 0;
    cfgInputParams[0].redeemFine = 0;
    cfgInputParams[0].redeemThreshold = 0;
    cfgInputParams[0].minBuyItNowFine = 0;
    await configurator.batchConfigNft(cfgInputParams);
  ...
  it("Batch Activates the BAYC NFT as buyItNow", async () => {
    const { configurator, dataProvider, bayc } = testEnv;

    cfgInputParams[0].asset = bayc.address;
    cfgInputParams[0].buyItNowDuration = 1;
    cfgInputParams[0].redeemDuration = 1;
    cfgInputParams[0].redeemFine = 100;
    cfgInputParams[0].redeemThreshold = 5000;
    cfgInputParams[0].minBuyItNowFine = 2000;
    await configurator.batchConfigNft(cfgInputParams);
    ...
    expect(redeemFine).to.be.equal(100);
    expect(redeemThreshold).to.be.equal(5000);
    expect(minBuyItNowFine).to.be.equal(2000);
  });
