...
makeSuite("Subgraph tests", async (testEnv) => {
  ...
  before("Initializing configuration", async () => {
    ...
    saveBaycAssetPrice = (
      await testEnv.nftOracle.getAssetPrice(testEnv.bayc.address)
    ).toString();
  });
  after("Reset", () => {
    // Reset BigNumber
  ...
  it("borrow-buyItNow-redeem", async () => {
    ...
    const { oldNftPrice, newNftPrice } = await setNftAssetPriceForDebt(
      testEnv,
      "BAYC",
      "WETH",
      "10",
    ...
    const buyItNowSellPrice = new BigNumber(newNftPrice)
      .multipliedBy(1.1)
      .toFixed(0);
    const buyItNowSellAmount = await convertToCurrencyUnits(
      weth.address,
      buyItNowSellPrice
    );
  ...
  it("borrow-buyItNow-liquidate", async () => {
    ...
    await increaseBuyItNowDuration(testEnv, "BAYC", true);
