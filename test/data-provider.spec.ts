...
makeSuite("DataProvider", (testEnv) => {
  ...
  it("Query Wallet Reserve Data", async () => {
...

    {
      ...
      const cTokenBalance = await walletProvider.balanceOfReserve(
        depositor.address,
        tokenData.cTokenAddress
      );
      expect(cTokenBalance).to.be.gt(0); // all weth has deposited
    }
  ...
  it("Query Wallet NFT Data", async () => {
...

    {
      ...
      const cNFTBalance = await walletProvider.balanceOfNft(
        borrower.address,
        tokenData.cNFTAddress
      );
      expect(cNFTBalance).to.be.equal(1); // NFT 101 has used for borrow
    }

    {
      ...
      expect(cNFTBalance).to.be.equal(0);
    }
  ...
  it("Batch Query Wallet NFT Token by index", async () => {
    ...
    await waitForTx(
      await bayc.connect(depositor.signer).mint(depositorTokenId)
    );
    await waitForTx(await bayc.connect(borrower.signer).mint(borrowerTokenId));

    const depositorTokenIdsBayc = await walletProvider.batchTokenOfOwnerByIndex(
      depositor.address,
      bayc.address
    );
    ...
    const borrowerTokenIdsBayc = await walletProvider.batchTokenOfOwnerByIndex(
      borrower.address,
      bayc.address
    );
    ...
    const borrowerTokenIdsBBayc = await walletProvider.batchTokenOfOwnerByIndex(
      borrower.address,
      bBAYC.address
    );
  ...
  it("Batch Query Wallet NFT Token by owner", async () => {
    ...
    console.log(
      "depositorTokenIdsBayc.length = " + depositorTokenIdsBayc.length
    );
    expect(depositorTokenIdsBayc.length).to.be.equal(1); // mint NFT in previous test step
    console.log("depositorTokenIdsBayc[0] = " + depositorTokenIdsBayc[0]);
    console.log("depositorTokenId = " + depositorTokenId);
    expect(depositorTokenIdsBayc[0]).to.be.equal(depositorTokenId);
    ...
    console.log("borrowerTokenIdsBayc.length = " + borrowerTokenIdsBayc.length);
    expect(borrowerTokenIdsBayc.length).to.be.equal(2); // NFT 102 not used for borrow, and nft at previous test step
    console.log("borrowerTokenIdsBayc[1] = " + borrowerTokenIdsBayc[1]);
    console.log("borrowerTokenId = " + borrowerTokenId);
    expect(borrowerTokenIdsBayc[1]).to.be.equal(borrowerTokenId);
    ...
    console.log(
      "borrowerTokenIdsBBayc.length = " + borrowerTokenIdsBBayc.length
    );
    expect(borrowerTokenIdsBBayc.length).to.be.equal(1); // NFT 101 has used for borrow
  });
