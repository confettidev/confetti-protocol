...
makeSuite("PunkGateway: Batch borrow", (testEnv: TestEnv) => {
  ...
  it("Batch Borrow USDC", async () => {
    ...
    await waitForTx(
      await cryptoPunksMarket.connect(borrower.signer).getPunk(punkIndex1)
    );
    await waitForTx(
      await cryptoPunksMarket
        .connect(borrower.signer)
        .offerPunkForSaleToAddress(punkIndex1, 0, punkGateway.address)
    );
    await waitForTx(
      await cryptoPunksMarket.connect(borrower.signer).getPunk(punkIndex2)
    );
    await waitForTx(
      await cryptoPunksMarket
        .connect(borrower.signer)
        .offerPunkForSaleToAddress(punkIndex2, 0, punkGateway.address)
    );
    ...
    await waitForTx(
      await punkGateway
        ...
        .batchRepay(
          [punkIndex1, punkIndex2],
          [borrowSize1.div(2), borrowSize1.div(2)]
        )
    );

    const loanData1AfterRepayPart = await dataProvider.getLoanDataByCollateral(
      wrappedPunk.address,
      punkIndex1
    );
    expect(loanData1AfterRepayPart.state).to.be.eq(ProtocolLoanState.Active);

    const loanData2AfterRepayPart = await dataProvider.getLoanDataByCollateral(
      wrappedPunk.address,
      punkIndex2
    );
    ...
    const loanData1AfterRepayFull = await dataProvider.getLoanDataByLoanId(
      loanData1AfterRepayPart.loanId
    );
    expect(loanData1AfterRepayFull.state).to.be.eq(ProtocolLoanState.Repaid);

    const loanData2AfterRepayFull = await dataProvider.getLoanDataByLoanId(
      loanData2AfterRepayPart.loanId
    );
  ...
  it("Batch Borrow ETH", async () => {
    ...
    expect(userBalanceAfterBorrow).to.be.gte(
      userBalanceBeforeBorrow.add(borrowSizeAll).sub(gasCost)
    );

    const loanData1AfterBorrow = await dataProvider.getLoanDataByCollateral(
      wrappedPunk.address,
      punkIndex1
    );
    expect(loanData1AfterBorrow.state).to.be.eq(ProtocolLoanState.Active);

    const loanData2AfterBorrow = await dataProvider.getLoanDataByCollateral(
      wrappedPunk.address,
      punkIndex1
    );
    ...
    const repaySize1Full = new BigNumber(
      loanData1AfterRepayPart.currentAmount.toString()
    );
    const repaySize2Full = new BigNumber(
      loanData2AfterRepayPart.currentAmount.toString()
    );
    await waitForTx(
      await punkGateway
        .connect(borrower.signer)
