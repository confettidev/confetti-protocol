...
makeSuite("PunkGateway", (testEnv: TestEnv) => {
  ...
  before("Initializing configuration", async () => {
    ...
    calculationsConfiguration.reservesParams = <
      iConfettiPoolAssets<IReserveParams>
    >getReservesConfigByPool(ConfettiPools.proto);
  });
  after("Reset", () => {
    // Reset BigNumber
    BigNumber.config({
  ...
  it("Owner can do emergency CryptoPunks recovery", async () => {
    ...
    await waitForTx(
      await cryptoPunksMarket
        .connect(user.signer)
        .transferPunk(punkGateway.address, punkIndex)
    );
    const tokenOwnerAfterBadTransfer =
      await cryptoPunksMarket.punkIndexToAddress(punkIndex);
    expect(tokenOwnerAfterBadTransfer).to.be.eq(
      punkGateway.address,
      "User should have lost the punk here."
    );
    ...
    const tokenOwnerAfterRecovery = await cryptoPunksMarket.punkIndexToAddress(
      punkIndex
    );

    expect(tokenOwnerAfterRecovery).to.be.eq(
      user.address,
      "User should recover the punk due emergency transfer"
    );
  });
  ...
  it("Borrow some USDC and repay it", async () => {
    ...
    await waitForTx(
      await usdc
        ...
        .mint(repaySize.sub(borrowSizeAll).toString())
    );
    ...
    const usdcBalanceBefore = await getERC20TokenBalance(
      usdc.address,
      borrower.address
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(borrower.signer)
        .borrow(usdc.address, borrowSize1, punkIndex, borrower.address, "0")
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(borrower.signer)
        .borrow(usdc.address, borrowSize2, punkIndex, borrower.address, "0")
    );

    const usdcBalanceAfterBorrow = await getERC20TokenBalance(
      usdc.address,
      borrower.address
    );
    ...
    expect(usdcBalanceAfterBorrow).to.be.gte(
      usdcBalanceBefore.add(borrowSizeAll)
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(borrower.signer)
        .repay(punkIndex, repaySize.div(2))
    );
    const usdcBalanceAfterPartialRepay = await getERC20TokenBalance(
      usdc.address,
      borrower.address
    );
    ...
    expect(
      await getWrappedPunkOwner(),
      "WrappedPunk should owned by loan after partial borrow"
    ).to.be.eq(wrapperPunkOwner);
    ...
    await waitForTx(
      await punkGateway.connect(borrower.signer).repay(punkIndex, repaySize)
    );
    const usdcBalanceAfterFullRepay = await getERC20TokenBalance(
      usdc.address,
      borrower.address
    );
  ...
  it("Borrow all USDC and repay it", async () => {
    ...
    const secondsToTravel = new BigNumber(365)
    ...
    const repaySize = borrowSize.plus(
      borrowSize.multipliedBy(5).dividedBy(100)
    );
    const punkIndex = testEnv.punkIndexTracker++;
    ...
    // Check results
    const loanDataAfterBorrow = await dataProvider.getLoanDataByCollateral(
      wrappedPunk.address,
      punkIndex
    );
    ...
    await waitForTx(
      await punkGateway.connect(user.signer).repay(punkIndex, MAX_UINT_AMOUNT)
    );

    // Check results
    const loanDataAfterRepayFull = await dataProvider.getLoanDataByLoanId(
      loanDataAfterBorrow.loanId
    );
  ...
  it("Borrow some ETH and repay it", async () => {
    ...
    await waitForTx(
      await punkGateway
        .connect(user.signer)
        .borrowETH(borrowSize1, punkIndex, user.address, "0")
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(user.signer)
        .borrowETH(borrowSize2, punkIndex, user.address, "0")
    );
    ...
    expect(
      await user.signer.getBalance(),
      "current eth balance shoud increase"
    ).to.be.gt(ethBalanceBefore);
    expect(debtAfterBorrow, "debt should gte borrowSize").to.be.gte(
      borrowSizeAll
    );
    ...
    await waitForTx(
      await punkGateway
        .connect(user.signer)
        .repayETH(punkIndex, repaySize.div(2), {
          value: repaySize.div(2),
        })
    );
    const loanDataAfterRepayPart = await dataProvider.getLoanDataByLoanId(
      loanDataAfterBorrow.loanId
    );
    const debtAfterPartialRepay = await getDebtBalance();
    ...
    await waitForTx(
      await punkGateway
        .connect(user.signer)
        .repayETH(punkIndex, MAX_UINT_AMOUNT, {
          value: repaySize,
        })
    );
    const debtAfterFullRepay = await getDebtBalance();
  ...
  it("Borrow all ETH and repay it", async () => {
    ...
    await waitForTx(
      await punkGateway
        .connect(user.signer)
        .borrowETH(borrowSize.toFixed(0), punkIndex, user.address, "0")
    );
