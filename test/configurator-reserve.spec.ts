...
import { strategyWETH } from "../markets/confetti/reservesConfigs";
...
makeSuite("Configurator-Reserve", (testEnv: TestEnv) => {
  ...
  it("Reverts trying to set an invalid reserve factor", async () => {
    ...
    await expect(configurator.batchConfigReserve(inputs)).to.be.revertedWith(
      RC_INVALID_RESERVE_FACTOR
    );
  });
  ...
  it("Check the onlyAdmin on deactivateReserve ", async () => {
    ...
    await expect(
      configurator
        ...
        .setActiveFlagOnReserve([weth.address], false),
      CALLER_NOT_POOL_ADMIN
  ...
  it("Check the onlyAdmin on activateReserve ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .setActiveFlagOnReserve([weth.address], true),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Check the onlyAdmin on freezeReserve ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .setFreezeFlagOnReserve([weth.address], true),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Check the onlyAdmin on unfreezeReserve ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .setFreezeFlagOnReserve([weth.address], false),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Check the onlyAdmin on disableBorrowingOnReserve ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .setBorrowingFlagOnReserve([weth.address], false),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Check the onlyAdmin on enableBorrowingOnReserve ", async () => {
    ...
    await expect(
      configurator
        .connect(users[2].signer)
        .setBorrowingFlagOnReserve([weth.address], true),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
  ...
  it("Changes the reserve factor of WETH", async () => {
    ...
    const { reserveFactor } = await dataProvider.getReserveConfigurationData(
      weth.address
    );
  ...
  it("Config setMaxNumberOfReserves invalid value", async () => {
    const { configurator, users, pool } = testEnv;
    await expect(
      configurator.setMaxNumberOfReserves(2),
      LPC_INVALID_CONFIGURATION
