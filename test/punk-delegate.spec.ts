...
makeSuite("PunkGateway: Delegate", (testEnv: TestEnv) => {
  ...
  it("Hacker try to borrow and delegate different onBehalf (should revert)", async () => {
    ...
    await expect(
      punkGateway
        ...
        .borrow(weth.address, borrowSize2, tokenId, borrower.address, "0")
    ...
    await expect(
      punkGateway
        .connect(hacker.signer)
        .borrowETH(borrowSize2, tokenId, borrower.address, "0")
    ).to.be.revertedWith(ProtocolErrors.CALLER_NOT_ONBEHALFOF_OR_IN_WHITELIST);
  });
  ...
  it("Hacker try to auction and delegate different onBehalf (should revert)", async () => {
    ...
    await expect(
      punkGateway
        .connect(hacker.signer)
        .buyItNow(tokenId, "1000000", liquidator.address)
    ).to.be.revertedWith(ProtocolErrors.CALLER_NOT_ONBEHALFOF_OR_IN_WHITELIST);
    ...
    await expect(
      punkGateway
        .connect(hacker.signer)
        .auctionETH(tokenId, liquidator.address, { value: depositSize })
    ).to.be.revertedWith(ProtocolErrors.CALLER_NOT_ONBEHALFOF_OR_IN_WHITELIST);
  });
});
