...
import { CommonsConfig } from "../markets/confetti/commons";
import { waitForTx } from "../helpers/misc-utils";

makeSuite("CToken", (testEnv: TestEnv) => {
  const {
  ...
  it("Check DAI basic parameters", async () => {
    ...
    expect(bSymbol).to.be.equal(CommonsConfig.CTokenSymbolPrefix + symbol);

    //const name = await dai.name();
    const bName = await bDai.name();
    expect(bName).to.be.equal(CommonsConfig.CTokenNamePrefix + " " + symbol);
  ...
  it("User 0 deposits 1000 DAI, transfers bDAI to user 1", async () => {
    ...
    const checkResult =
      await testEnv.mockIncentivesController.checkHandleActionIsCalled();
    await waitForTx(
      await testEnv.mockIncentivesController.resetHandleActionIsCalled()
    );
