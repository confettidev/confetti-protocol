...
makeSuite("LendPool: Redeem", (testEnv) => {
  ...
  it("WETH - Sells via buy-it-now the borrow", async () => {
    const { weth, bayc, bBAYC, users, pool, dataProvider } = testEnv;
    const liquidator = users[3];
    const borrower = users[1];

    //mints WETH to the liquidator
    // liquidator is a user who activates the auction
    // why is the liquidator minting 1000 WETH?
    ...
    // The BuyItNow price will be 1.1 times the liquidate price
    const buyItNowSellPrice = new BigNumber(liquidatePrice.toString()).multipliedBy(1.1).toFixed(0);
    // Does this create the auction or does this liquidate the NFT? This liquidates the NFT
    // Here, pool == LendPool. If this is all that's needed then how do I remove the bidding?
    ...
    expect(lendPoolBalanceAfter).to.be.equal(
      ...
      "Invalid lend pool balance after buyItNow"
    );

    const loanDataAfter = await dataProvider.getLoanDataByLoanId(loanDataBefore.loanId);
    expect(loanDataAfter.state).to.be.equal(ProtocolLoanState.BuyItNow, "Invalid loan state after buyItNow");
  ...
  it("WETH - Redeems the borrow", async () => {
    ...
    const buyItNowDataBefore = await pool.getNftBuyItNowData(bayc.address, "101");
    ...
    const buyItNowFineAmount = new BigNumber(buyItNowDataBefore.buyItNowFine.toString()).multipliedBy(1.1).toFixed(0);

    await pool.connect(borrower.signer).redeem(bayc.address, "101", repayDebtAmount, buyItNowFineAmount);
    ...
    expect(liquidatorBalanceAfter).to.be.gte(
      liquidatorBalanceBefore.add(buyItNowDataBefore.buyItNowFine),
  ...
  it("USDC - BuyItNow the borrow", async () => {
  ...
  it("USDC - Redeems the borrow", async () => {
    ...
    await pool.connect(borrower.signer).redeem(bayc.address, "102", repayDebtAmount, buyItNowFineAmount);
