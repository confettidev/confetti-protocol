...
import {
  ...
  calcExpectedReserveDataAfterBuyItNow,
  ...
  calcExpectedUserDataAfterBuyItNow,
  ...
  calcExpectedLoanDataAfterBuyItNow,
...
export const setNftAssetPriceForDebt = async (
  ...
  const priceAdmin = await getEthersSignerByAddress(
    await nftOracle.priceFeedAdmin()
  );
  ...
  const { liquidationThreshold } = await dataProvider.getNftConfigurationData(
    nftAsset
  );
  ...
  await waitForTx(
    await nftOracle
      .connect(priceAdmin)
      .setAssetData(nftAsset, nftPrice.toFixed(0))
  );
...
export const setNftAssetPrice = async (
  ...
  await waitForTx(
    await nftOracle
      .connect(priceAdmin)
      .setAssetData(nftAsset, priceBN.toFixed(0))
  );
  await advanceTimeAndBlock(100);
  await waitForTx(
    await nftOracle
...
export const increaseRedeemDuration = async (
  ...
  if (isEnd) {
    await increaseTime(
      nftCfgData.redeemDuration.mul(ONE_DAY).add(ONE_HOUR).toNumber()
    );
  } else {
    await increaseTime(
      nftCfgData.redeemDuration.mul(ONE_DAY).sub(ONE_HOUR).toNumber()
    );
  }
};

export const increaseBuyItNowDuration = async (
  testEnv: TestEnv,
  nftSymbol: string,
  isEnd: Boolean
) => {
  ...
  if (isEnd) {
    await increaseTime(
      nftCfgData.buyItNowDuration.mul(ONE_DAY).add(ONE_HOUR).toNumber()
    );
  } else {
    await increaseTime(
      nftCfgData.buyItNowDuration.mul(ONE_DAY).sub(ONE_HOUR).toNumber()
    );
  }
};
...
export const buyItNow = async (
  ...
  let amountToBuyItNow = (
    await convertToCurrencyDecimals(reserveAsset, price)
  ).toString();

  amountToBuyItNow = "0x" + new BigNumber(amountToBuyItNow).toString(16);

  if (isFirstTime && expectedResult === "success") {
    const txResult = await waitForTx(
      await pool
        .connect(user.signer)
        .buyItNow(nftAsset, nftTokenId, amountToBuyItNow, onBehalfOf.address)
    );
    ...
    const expectedReserveData = calcExpectedReserveDataAfterBuyItNow(
      amountToBuyItNow,
      reserveDataBefore,
      userDataBefore,
      loanDataBefore,
    ...
    const expectedUserData = calcExpectedUserDataAfterBuyItNow(
      reserveDataBefore,
      expectedReserveData,
      userDataBefore,
      loanDataBefore,
    ...
    const expectedLoanData = calcExpectedLoanDataAfterBuyItNow(
      amountToBuyItNow,
      reserveDataBefore,
      expectedReserveData,
      loanDataBefore,
...
export const redeem = async (
  ...
  let buyItNowFineAmount = loanDataBefore.buyItNowBorrowAmount
    .multipliedBy(1.1)
    .toFixed(0);
  ...
  if (expectedResult === "success") {
    const txResult = await waitForTx(
      await pool
        .connect(user.signer)
        .redeem(nftAsset, nftTokenId, amountToRedeem, buyItNowFineAmount)
    );
...
const getDataBeforeAction = async (
  ...
  const cTokenInstance = await getCToken(reserveData.cTokenAddress);
  return {
    reserve,
    reserveData,
    userData,
