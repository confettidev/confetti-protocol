...
makeSuite("WETHGateway", (testEnv: TestEnv) => {
  ...
  it("Deposit WETH via WethGateway ", async () => {
    ...
    const cTokensBalance = await bWETH.balanceOf(user.address);

    expect(cTokensBalance).to.be.gt(zero);
    expect(cTokensBalance).to.be.gte(depositSize);
  });
  ...
  it("Withdraw WETH - Partial", async () => {
    ...
    expect(cTokensBalance).to.be.gt(zero, "User should have cTokens.");

    // Partially withdraw native ETH
    const partialWithdraw = await convertToCurrencyDecimals(bWETH.address, "2");

    // Approve the cTokens to Gateway so Gateway can withdraw and convert to Ether
    await waitForTx(
      await bWETH
        .connect(user.signer)
    ...
    const afterPartialCTokensBalance = await bWETH.balanceOf(user.address);
    ...
    expect(afterPartialCTokensBalance).to.be.equal(
      cTokensBalance.sub(partialWithdraw),
  ...
  it("Withdraw WETH - Full", async () => {
    ...
    const afterFullCTokensBalance = await bWETH.balanceOf(user.address);
    ...
    expect(afterFullEtherBalance).to.be.lt(
      priorEthersBalance.add(cTokensBalance),
      "User ETHER balance should less than before balance + withdraw"
    );
    expect(afterFullCTokensBalance).to.be.eq(
      0,
      "User bWETH balance should be zero"
    );
  });

  it("Borrow WETH and Full Repay with ETH", async () => {
    const { users, wethGateway, pool, loan, bWETH, bayc, dataProvider } =
      testEnv;
    const depositor = users[0];
    const user = users[1];
    ...
    const cTokensBalance = await bWETH.balanceOf(depositor.address);
    expect(cTokensBalance).to.be.gte(depositSize);
    ...
    await waitForTx(
      await wethGateway
        .connect(user.signer)
        .repayETH(nftAsset, tokenId, partialPayment, {
    ...
    await waitForTx(
      await wethGateway
        .connect(user.signer)
        .repayETH(nftAsset, tokenId, MAX_UINT_AMOUNT, {
          value: repaySize,
        })
    );
  ...
  it("Borrow ETH and Full Repay with ETH", async () => {
    const { users, wethGateway, pool, loan, weth, bWETH, bayc, dataProvider } =
      testEnv;
    const depositor = users[0];
    ...
    const cTokensBalance = await bWETH.balanceOf(depositor.address);
    expect(cTokensBalance, "cTokensBalance not gte depositSize").to.be.gte(
      depositSize
    );
    ...
    await waitForTx(
      await wethGateway
        .connect(borrower.signer)
        .borrowETH(borrowSize1, nftAsset, tokenId, borrower.address, "0")
    );
    ...
    await waitForTx(
      await wethGateway
        .connect(borrower.signer)
        .borrowETH(borrowSize2, nftAsset, tokenId, borrower.address, "0")
    );
  ...
  it("Batch Borrow ETH", async () => {
    ...
    const loanData1AfterBorrow = await getLoanData(
      pool,
      dataProvider,
      nftAsset,
      tokenId1,
    ...
    const loanData2AfterBorrow = await getLoanData(
      pool,
      dataProvider,
      nftAsset,
      tokenId2,
    ...
    expect(loanData1AfterBorrow.state.toNumber()).to.be.eq(
      ProtocolLoanState.Active
    );
    expect(loanData2AfterBorrow.state.toNumber()).to.be.eq(
      ProtocolLoanState.Active
    );
    ...
    const loanData1AfterRepayPart = await getLoanData(
      pool,
      dataProvider,
      nftAsset,
      tokenId1,
    ...
    const loanData2AfterRepayPart = await getLoanData(
      pool,
      dataProvider,
      nftAsset,
      tokenId2,
    ...
    expect(loanData1AfterRepayPart.state.toNumber()).to.be.eq(
      ProtocolLoanState.Active
    );
    expect(loanData2AfterRepayPart.state.toNumber()).to.be.eq(
      ProtocolLoanState.Active
    );
    ...
    expect(loanData1AfterRepayFull.state.toNumber()).to.be.eq(
      ProtocolLoanState.Repaid
    );
    expect(loanData2AfterRepayFull.state.toNumber()).to.be.eq(
      ProtocolLoanState.Repaid
    );
  });
