...
makeSuite("WETHGateway - Liquidate", (testEnv: TestEnv) => {
  ...
  before("Initializing configuration", async () => {
    ...
    depositSize = new BigNumber(baycInitPrice.toString())
      .multipliedBy(0.8)
      .toFixed(0);
  });
  ...
  it("Borrow ETH and Liquidate it", async () => {
...

    {
      ...
      await waitForTx(
        await nftOracle.setAssetData(bayc.address, baycInitPrice)
      );
    }
    ...
    await waitForTx(
      await wethGateway
        .connect(user.signer)
        .borrowETH(amountBorrow, nftAsset, tokenId, user.address, "0")
    );
    ...
    const nftDebtDataBeforeBuyItNow = await pool.getNftDebtData(
      bayc.address,
      tokenId
    );
    expect(
      nftDebtDataBeforeBuyItNow.healthFactor.toString()
    ).to.be.bignumber.lt(oneEther.toFixed(0));
    ...
    const buyItNowAmountSend = new BigNumber(liquidatePrice.toString())
      .multipliedBy(1.05)
      .toFixed(0);
    console.log("buyItNowETH:", liquidatePrice.toString(), buyItNowAmountSend);
    await waitForTx(
      await wethGateway
        .connect(liquidator.signer)
        .buyItNowETH(nftAsset, tokenId, liquidator.address, {
          value: buyItNowAmountSend,
        })
    );

    await increaseTime(
      nftCfgData.buyItNowDuration.mul(ONE_DAY).add(100).toNumber()
    );

    await increaseTime(new BigNumber(ONE_DAY).multipliedBy(365).toNumber()); // accrue more interest, debt exceed buyItNow price
    ...
    const extraAmount = new BigNumber(
      loanDataBeforeLiquidate.currentAmount
        .sub(loanDataBeforeLiquidate.buyItNowPrice)
        .toString()
    )
      .multipliedBy(1.1)
      .toFixed(0);
    ...
    await waitForTx(
      await wethGateway
        .connect(liquidator.signer)
        .liquidateETH(nftAsset, tokenId, { value: extraAmount })
    );

    const loanDataAfter = await dataProvider.getLoanDataByLoanId(
      nftDebtDataBeforeBuyItNow.loanId
    );
  ...
  it("Borrow ETH and Redeem it", async () => {
    ...
    console.log("buyItNowETH:", liquidatePrice, liquidateAmountSend);
    await waitForTx(
      await wethGateway
        .connect(liquidator.signer)
        .buyItNowETH(nftAsset, tokenId, liquidator.address, {
    ...
    await increaseTime(
      nftCfgData.buyItNowDuration.mul(ONE_DAY).sub(100).toNumber()
    );
    const buyItNowData = await pool.getNftBuyItNowData(nftAsset, tokenId);
    const buyItNowFineAmount = new BigNumber(
      buyItNowData.buyItNowFine.toString()
    )
      .multipliedBy(1.1)
      .toFixed(0);
    const repayAmount = new BigNumber(
      buyItNowData.buyItNowBorrowAmount.toString()
    )
      .multipliedBy(0.51)
      .toFixed(0);
    const redeemAmountSend = new BigNumber(repayAmount)
      .plus(buyItNowFineAmount)
      .toFixed(0);
    console.log("redeemETH:", redeemAmountSend.toString());
    await waitForTx(
      await wethGateway
        .connect(user.signer)
        .redeemETH(nftAsset, tokenId, repayAmount, buyItNowFineAmount, {
    ...
    await waitForTx(
      await wethGateway
        .connect(user.signer)
        .repayETH(nftAsset, tokenId, redeemAmountSend, {
          value: redeemAmountSend,
        })
    );
