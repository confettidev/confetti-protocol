 ...
 * @author Confetti, influenced by https://github.com/wbobeirne/eth-balance-checker/blob/master/contracts/BalanceChecker.sol
 * @notice Implements a logic of getting multiple tokens balance for one user address
 * @dev NOTE: THIS CONTRACT IS NOT USED WITHIN THE Confetti PROTOCOL. It's an accessory contract used to reduce the number of calls
 * towards the blockchain from the Confetti backend.
 **/
contract WalletBalanceProvider {
    ...
    **/
    function balanceOfReserve(address user, address token)
        public
        view
        returns (uint256)
    {
    ...
    function batchBalanceOfReserve(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                balances[i * tokens.length + j] = balanceOfReserve(
                    users[i],
                    tokens[j]
                );
            }
    ...
    */
    ...
    Returns the balance of the nft for user. Avoids possible errors:
      - return 0 on non-contract address
    **/
    function balanceOfNft(address user, address token)
        public
        view
        returns (uint256)
    {
    ...
    function batchBalanceOfNft(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                balances[i * tokens.length + j] = balanceOfNft(
                    users[i],
                    tokens[j]
                );
            }
    ...
    */
     ...
     */
    function batchTokenOfOwnerByIndex(address owner, address token)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 index = 0; index < tokenBalances; index++) {
            tokenIds[index] = IERC721Enumerable(token).tokenOfOwnerByIndex(
                owner,
                index
            );
        }
    ...
    function batchPunkOfOwner(
        ...
        for (uint256 punkIndex = 0; punkIndex < maxIndex; punkIndex++) {
            address ownerAddress = IPunks(punkContract).punkIndexToAddress(
                punkIndex
            );
