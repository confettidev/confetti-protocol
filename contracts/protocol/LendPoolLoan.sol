    ...
    IERC721ReceiverUpgradeable
{
    ...
    function buyItNowLoan(
        ...
        // Must use storage to change state
        DataTypes.LoanData storage loan = _loans[loanId];
        // TODO: remove instance of previous bidders. not needed for buy-it-now functionality
        // address previousBidder = loan.bidderAddress;
        // uint256 previousPrice = loan.bidPrice;

        // Ensure valid loan state
        if (loan.buyItNowStartTimestamp == 0) {
            require(
                loan.state == DataTypes.LoanState.Active,
                Errors.LPL_INVALID_LOAN_STATE
            );

            loan.state = DataTypes.LoanState.BuyItNow;
            loan.buyItNowStartTimestamp = block.timestamp;
            // TODO: update the LoanData Datatype to remove firstBidderAddress and replace with buyItNowBuyerAddress
            loan.buyItNowBuyerAddress = onBehalfOf;
        } else {
            require(
                loan.state == DataTypes.LoanState.BuyItNow,
                Errors.LPL_INVALID_LOAN_STATE
            );
            // TODO: This next line can likely be removed since it is for updating bid price.
            require(
        ...
        loan.buyItNowBorrowAmount = borrowAmount;
        loan.buyItNowBuyerAddress = onBehalfOf;
        loan.buyItNowPrice = buyItNowPrice;
        loan.state = DataTypes.LoanState.BuyItNow;
    ...
    function redeemLoan(
        ...
        loan.state = DataTypes.LoanState.Active;
        loan.buyItNowStartTimestamp = 0;
        loan.buyItNowBorrowAmount = 0;
        loan.buyItNowBuyerAddress = address(0);
        loan.buyItNowPrice = 0;
        // loan.firstBuyItNowBuyerAddress = address(0);
    ...
    function liquidateLoan(
        ...
        _loans[loanId].buyItNowBorrowAmount = borrowAmount;
    ...
    {
        return (
            _loans[loanId].buyItNowBuyerAddress,
            _loans[loanId].buyItNowPrice
        );
    }
