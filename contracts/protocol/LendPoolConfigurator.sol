 ...
 * @author Confetti
 * @dev Implements the configuration methods for the Confetti protocol
 **/
...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
     ...
     **/
    function batchInitReserve(ConfigTypes.InitReserveInput[] calldata input)
        external
        onlyPoolAdmin
    {
    ...
    function batchInitNft(ConfigTypes.InitNftInput[] calldata input)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
     ...
     **/
    function updateCToken(ConfigTypes.UpdateCTokenInput[] calldata inputs)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
        ...
        for (uint256 i = 0; i < inputs.length; i++) {
            ConfiguratorLogic.executeUpdateCToken(cachedPool, inputs[i]);
        }
    }
     ...
     **/
    function updateDebtToken(ConfigTypes.UpdateDebtTokenInput[] calldata inputs)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function setBorrowingFlagOnReserve(address[] calldata assets, bool flag)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
        for (uint256 i = 0; i < assets.length; i++) {
            DataTypes.ReserveConfigurationMap memory currentConfig = cachedPool
                .getReserveConfiguration(assets[i]);
    ...
    function setActiveFlagOnReserve(address[] calldata assets, bool flag)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function setFreezeFlagOnReserve(address[] calldata assets, bool flag)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
     ...
     **/
    function setReserveFactor(address[] calldata assets, uint256 reserveFactor)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function batchConfigReserve(ConfigReserveInput[] calldata inputs)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function setActiveFlagOnNft(address[] calldata assets, bool flag)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function setFreezeFlagOnNft(address[] calldata assets, bool flag)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    /**
     * @dev Configures the NFT buy-it-now parameters
     * @param assets The address of the underlying asset of the reserve
     * @param redeemDuration The threshold at which loans using this asset as collateral will be considered undercollateralized
     * @param buyItNowDuration The bonus liquidators receive to liquidate this asset.
     **/
    function configureNftAsBuyItNow(
        address[] calldata assets,
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            //only be lower or equal than the buy-it-now duration
            require(
                redeemDuration <= buyItNowDuration,
                Errors.LPC_INVALID_CONFIGURATION
            );

            currentConfig.setRedeemDuration(redeemDuration);
            currentConfig.setBuyItNowDuration(buyItNowDuration);
            currentConfig.setRedeemFine(redeemFine);

            cachedPool.setNftConfiguration(assets[i], currentConfig.data);

            emit NftBuyItNowChanged(
                assets[i],
                redeemDuration,
                buyItNowDuration,
                redeemFine
    ...
    function setNftMinBuyItNowFine(
        address[] calldata assets,
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            currentConfig.setMinBuyItNowFine(minBuyItNowFine);

            cachedPool.setNftConfiguration(assets[i], currentConfig.data);

            emit NftMinBuyItNowFineChanged(assets[i], minBuyItNowFine);
        }
    }

    function batchConfigNft(ConfigNftInput[] calldata inputs)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
        for (uint256 i = 0; i < inputs.length; i++) {
            DataTypes.NftConfigurationMap memory currentConfig = cachedPool
                .getNftConfiguration(inputs[i].asset);
            ...
            emit NftMinBuyItNowFineChanged(
                inputs[i].asset,
                inputs[i].minBuyItNowFine
            );
        }
    }
    ...
    /**
     * @dev pauses or unpauses all the actions of the protocol, including cToken transfers
