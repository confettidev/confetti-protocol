...
contract MaliciousHackerERC721 is IERC721Receiver {
  ...
  uint256 public constant ACTION_BUYITNOW = 5;
  ...
  function onERC721Received(
    ...
    vars.referralCode = 0;
    vars.amount = 1 ether;
    vars.buyItNowPrice = 2 ether;
    vars.buyItNowFine = 0.1 ether;
    ...
    } else if (_simulateAction == ACTION_BUYITNOW) {
      _pool.buyItNow(vars.nfts[0], tokenId, vars.buyItNowPrice, vars.onBehalfOf);
    } else if (_simulateAction == ACTION_REDEEM) {
      _pool.redeem(vars.nfts[0], tokenId, vars.amount, vars.buyItNowFine);
