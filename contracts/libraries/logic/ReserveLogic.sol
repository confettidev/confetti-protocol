...
library ReserveLogic {
    ...
    function updateState(DataTypes.ReserveData storage reserve) internal {
        uint256 scaledVariableDebt = IDebtToken(reserve.debtTokenAddress)
            .scaledTotalSupply();
    ...
    function updateInterestRates(
        ...
        vars.totalVariableDebt = IDebtToken(reserve.debtTokenAddress)
            .scaledTotalSupply()
            .rayMul(reserve.variableBorrowIndex);
    ...
    function _mintToTreasury(
        ...
        if (vars.amountToMint != 0) {
            ICToken(reserve.cTokenAddress).mintToTreasury(
                vars.amountToMint,
                newLiquidityIndex
            );
        }
