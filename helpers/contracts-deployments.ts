...
export const deployCNFTRegistry = async (verify?: boolean) => {
  const cnftRegistryImpl = await new CNFTRegistryFactory(
    await getDeploySigner()
  ).deploy();
  await insertContractAddressInDb(
    eContractid.CNFTRegistryImpl,
...
export const deployConfettiLibraries = async (verify?: boolean) => {
...
export const deployConfettiProtocolDataProvider = async (
  addressesProvider: tEthereumAddress,
  verify?: boolean
) =>
  withSaveAndVerify(
    await new ConfettiProtocolDataProviderFactory(
      await getDeploySigner()
    ).deploy(addressesProvider),
    eContractid.ConfettiProtocolDataProvider,
    [addressesProvider],
    verify
  );
...
export const deployGenericCTokenImpl = async (verify: boolean) =>
  withSaveAndVerify(
    await new CTokenFactory(await getDeploySigner()).deploy(),
    eContractid.CToken,
    [],
    verify
  );

export const deployGenericCNFTImpl = async (verify: boolean) =>
  withSaveAndVerify(
    await new CNFTFactory(await getDeploySigner()).deploy(),
    eContractid.CNFT,
    [],
    verify
  );
...
export const deployAllMockTokens = async (
  ...
  const protoConfigData = getReservesConfigByPool(ConfettiPools.proto);

  for (const tokenSymbol of Object.keys(TokenContractId)) {
    const tokenName = "Confetti Mock " + tokenSymbol;
...
export const deployAllMockNfts = async (verify?: boolean) => {
  ...
  for (const tokenSymbol of Object.keys(NftContractId)) {
    const tokenName = "Confetti " + tokenSymbol;
...
export const chooseCTokenDeployment = (id: eContractid) => {
  switch (id) {
    case eContractid.CToken:
      return deployGenericCTokenImpl;
    //case eContractid.DelegationAwareCToken:
    //  return deployDelegationAwareCTokenImpl;
    default:
      throw Error(`Missing cToken implementation deployment script for: ${id}`);
  }
};

export const deployCTokenImplementations = async (
  pool: ConfigNames,
  ...
  // Obtain the different CToken implementations of all reserves inside the Market config
  const tokenImplementations = [
    ...Object.entries(reservesConfig).reduce<Set<eContractid>>(
      (acc, [, entry]) => {
        acc.add(entry.cTokenImpl);
        return acc;
      },
      new Set<eContractid>()
    ),
  ...
  for (let x = 0; x < tokenImplementations.length; x++) {
    ...
    if (!notFalsyOrZeroAddress(tokenAddress)) {
      const deployImplementationMethod = chooseCTokenDeployment(
        tokenImplementations[x]
      );
      console.log(`Deploying CToken implementation`, tokenImplementations[x]);
...
export const chooseCNFTDeployment = (id: eContractid) => {
  switch (id) {
    case eContractid.CNFT:
      return deployGenericCNFTImpl;
    //case eContractid.DelegationAwareCNFT:
    //  return deployDelegationAwareCNFTImpl;
    default:
      throw Error(`Missing cNFT implementation deployment script for: ${id}`);
  }
};

export const deployCNFTImplementations = async (
  pool: ConfigNames,
  ...
  // Obtain the different CNFT implementations of all nfts inside the Market config
  const cNFTImplementations = [
    ...Object.entries(NftsConfig).reduce<Set<eContractid>>((acc, [, entry]) => {
      acc.add(entry.cNFTImpl);
      return acc;
    }, new Set<eContractid>()),
  ];

  for (let x = 0; x < cNFTImplementations.length; x++) {
    const cNFTAddress = getOptionalParamAddressPerNetwork(
      poolConfig[cNFTImplementations[x].toString()],
      network
    );
    if (!notFalsyOrZeroAddress(cNFTAddress)) {
      const deployImplementationMethod = chooseCNFTDeployment(
        cNFTImplementations[x]
      );
      console.log(`Deploying CNFT implementation`, cNFTImplementations[x]);
      await deployImplementationMethod(verify);
    }
  }
};
...
export const deployConfettiUpgradeableProxy = async (
  id: string,
  ...
  withSaveAndVerify(
    await new ConfettiUpgradeableProxyFactory(await getDeploySigner()).deploy(
      logic,
      admin,
      data
    ),
...
export const deployConfettiProxyAdmin = async (id: string, verify?: boolean) =>
  withSaveAndVerify(
    await new ConfettiProxyAdminFactory(await getDeploySigner()).deploy(),
    id,
    [],
    verify
  );
...
export const deployConfettiCollector = async (args: [], verify?: boolean) => {
  const ConfettiCollectorImpl = await new ConfettiCollectorFactory(
    await getDeploySigner()
  ).deploy();
  await insertContractAddressInDb(
    eContractid.ConfettiCollectorImpl,
    ConfettiCollectorImpl.address
  );
  return withSaveAndVerify(
    ConfettiCollectorImpl,
