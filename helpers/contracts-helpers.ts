...
export const tryGetContractAddressInDb = async (
  id: string
): Promise<tEthereumAddress> => {
  const contractAtDb = await getDb(DRE.network.name).get(`${id}`).value();
  if (contractAtDb?.address) {
...
export const verifyContract = async (
  ...
  if (id == eContractid.ConfettiUpgradeableProxy) {
    await verifyEtherscanContract(
      instance.address,
      args,
      "contracts/libraries/proxy/ConfettiUpgradeableProxy.sol:ConfettiUpgradeableProxy"
    );
  } else if (
    id == eContractid.ConfettiProxyAdminFund ||
    id == eContractid.ConfettiProxyAdminPool
  ) {
    await verifyEtherscanContract(
      instance.address,
      args,
      "contracts/libraries/proxy/ConfettiProxyAdmin.sol:ConfettiProxyAdmin"
    );
  } else {
